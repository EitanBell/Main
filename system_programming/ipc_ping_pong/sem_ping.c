...
static int PingPongSem(void)
{
	...
	ping_sem = sem_open(g_sem_name_1, O_CREAT | O_EXCL, S_IRWXU, 0);
	...
	pong_sem = sem_open(g_sem_name_2, O_CREAT | O_EXCL, S_IRWXU, 0);
	...
	if (FAIL == (sem_unlink(g_sem_name_1)))
    {
        perror("Semaphore 1 failed to unlink");
        return FAIL;
    }
	...
	if (FAIL == (sem_unlink(g_sem_name_2)))
    {
        perror("Semaphore 2 failed to unlink");
        return FAIL;
    }
